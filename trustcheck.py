...
What it catches (v0.1):
  ...
  2. MEMORIZATION — train 99% / eval 71% means expect ~71% in production,
                    not the number you screenshotted.
...
class TrustMonitor:
    ...
    def memorization(self) -> Optional[dict]:
        ...
        return {
            "train": last.train_metric,
            "eval": last.eval_metric,
            "gap": gap,
            "memorizing": gap > self.memorization_gap,
        }
    ...
    def verdict(self) -> Verdict:
        ...
        if mem and mem["memorizing"]:
            ...
            findings.append(
                ...
                f"(gap {mem['gap']:.4f}). The model is memorizing training data, "
                f"not learning the task."
            )
            actions.append(
