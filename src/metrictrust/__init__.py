# -*- coding: utf-8 -*-
"""
metrictrust — Should you trust your fine-tune's validation number?
...
What it catches (v0.1):
  ...
  2. MEMORIZATION — train 99% / eval 71% AND eval has stagnated means expect
                    ...
                    still climbing — common with LoRA — is deliberately NOT
                    flagged; that's normal, not memorization. The check
                    looks at the trend, not just a snapshot.)
...
Not to be confused with (different problem, different tools):
  - TrainCheck (OrderLab/UMich) — catches silent BUGS in the training
    process itself (corrupted checkpoints, library errors, invariant
    violations). We don't check execution correctness — we check whether a
    correctly-executed run's final number should be trusted.
  - EvalTrust — audits whether a difference between two finished MODEL
    EVALUATIONS is statistically real (significance, effect size). We don't
    compare models — we watch a single training run's curve over time.
  metrictrust sits at a third point: mid-training, single-run, "is THIS
  checkpoint's number honest."
...
class Verdict:
    ...
    def __str__(self) -> str:
        ...
        lines = []
        lines.append("=" * 62)
        lines.append(f"{icon} METRICTRUST VERDICT: {self.level.replace('_', ' ')}")
        lines.append("=" * 62)
...
class TrustMonitor:
    ...
    def __init__(
        ...
        # train - eval larger than this => memorization warning (gap size
        # alone is not sufficient — see memorization() below)
        memorization_gap: float = 0.10,
        # more drops than this => run counts as oscillating. 1 means "2+
        # meaningful drops = oscillating" — deliberately strict: a run that
        # lost ground twice is not something to trust blindly.
...
# ---------------------------------------------------------------------------
# Self-demo: run `python -m metrictrust` to see it catch a lying run
# ---------------------------------------------------------------------------
if __name__ == "__main__":
    ...
    print("\n\nDemo 4: fast LoRA-style convergence — NOT flagged as memorization\n")
    mon4 = TrustMonitor(higher_is_better=True)
    for step, tr, ev in [(0, .50, .40), (1, .85, .50), (2, .97, .58),
                         (3, .99, .66), (4, .99, .73)]:
        mon4.record(step=step, eval_metric=ev, train_metric=tr)
    print(mon4.verdict())
